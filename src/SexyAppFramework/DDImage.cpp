...
bool DDImage::PolyFill3D(const Point theVertices[],
						 int theNumVertices,
...
void DDImage::BltF(Image *theImage,
				   float theX,
				   float theY,
...
void DDImage::BltRotated(Image *theImage,
						 float theX,
						 float theY,
						 const Rect &theSrcRect,
						 const Rect &theClipRect,
						 ...
						 float theRotCenterY)
{
	...
	if (Check3D(this))
	{
		mDDInterface->mD3DInterface->BltRotated(theImage,
												theX,
												theY,
												&theClipRect,
												theColor,
...
void DDImage::StretchBlt(Image *theImage,
						 const Rect &theDestRectOrig,
						 const Rect &theSrcRectOrig,
						 const Rect &theClipRect,
						 const Color &theColor,
						 int theDrawMode,
						 bool fastStretch)
{
	...
	if (fastStretch)
	{
			...
			(!aSrcDDImage->mHasAlpha) && (aSrcDDImage->GetSurface() != NULL))
		{
			...
			RECT aDestRect = {theDestRect.mX,
							  theDestRect.mY,
							  theDestRect.mX + theDestRect.mWidth,
...
void DDImage::StretchBltMirror(Image *theImage,
							   const Rect &theDestRectOrig,
							   const Rect &theSrcRectOrig,
							   const Rect &theClipRect,
							   const Color &theColor,
...
void DDImage::BltMatrix(Image *theImage,
						float x,
						float y,
						...
						bool blend)
{
	...
	if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 32)
		aPixelFormat = 0x888;
	else if (mLockedSurfaceDesc.ddpfPixelFormat.dwRBitMask == 0xf800 &&
			 mLockedSurfaceDesc.ddpfPixelFormat.dwGBitMask == 0x07e0 &&
			 mLockedSurfaceDesc.ddpfPixelFormat.dwBBitMask == 0x001f)
		aPixelFormat = 0x565;
	else if (mLockedSurfaceDesc.ddpfPixelFormat.dwRBitMask == 0x7c00 &&
			 mLockedSurfaceDesc.ddpfPixelFormat.dwGBitMask == 0x03e0 &&
			 mLockedSurfaceDesc.ddpfPixelFormat.dwBBitMask == 0x001f)
		aPixelFormat = 0x555;
	else
		DBG_ASSERTE(FALSE);

	BltMatrixHelper(theImage,
					x,
					y,
					theMatrix,
					theClipRect,
					...
					theDrawMode,
					theSrcRect,
					mLockedSurfaceDesc.lpSurface,
					mLockedSurfaceDesc.lPitch,
					aPixelFormat,
					blend);
...
void DDImage::BltTrianglesTex(Image *theTexture,
...
void DDImage::FillScanLinesWithCoverage(Span *theSpans,
										int theSpanCount,
										const Color &theColor,
										int theDrawMode,
